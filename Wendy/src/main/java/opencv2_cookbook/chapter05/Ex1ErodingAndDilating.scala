...
object Ex1ErodingAndDilating extends App {
    ...
    val eroded = IplImage.create(cvGetSize(image), image.depth, 1)
    cvErode(image, eroded, null, 1)
    show(eroded, "Eroded")

    // Dilate image, by default 3x3 element is used
    val dilated = IplImage.create(cvGetSize(image), image.depth, 1)
    ...
    val kernel = cvCreateStructuringElementEx(
        ...
        CV_SHAPE_RECT, null)
    val eroded7x7 = IplImage.create(cvGetSize(image), image.depth, 1)
