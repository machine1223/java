...
//						}
				...
				if(returnObj)
				{
					...
					if (!returnObj.isvalid) {
						...
						if (thefirstid == ""){
							...
							//if($("#"+tipid).hasClass("onLoad"))
							//thefirsterrmsg="正在进行合法性校验，请稍后...";
						}
					}
	...
	ajaxValid : function(returnObj)
	{
		...
		if(setting.addidvalue)
		{
			...
			ls_url=ls_url+"?"+setting.createData();
		}
		//执行创建data的函数
		$.ajax(
		{	
			global:false,//不触发全局的Ajax事件
			mode : "abort",
			type : setting.type, 
			url : ls_url, 
			cache : setting.cache,
...
$.fn.ajaxValidator = function(controlOptions)
{
	var settings = 
	{
		beforeAjaxValid:function(){return true},//这个函数是自己所加，用来控制是否要进行ajax验证，当返回true时进行，否则不进行。
		isvalid : false,
		lastValid : "",
		type : "GET",
		url : "",
		addidvalue : true,
		datatype : "html",
		createData:function(){},//这个函数是刘冬所加，用来创建请求数据，应返回类似 name=liudong&age=24&gender=男 这样的字符串
								// （注意：这里的汉字没有编码，所以在配置tomcat服务器的时候，应指明URIEncoding="utf-8"）
		data : "",
		async : true,
		cache : false,
